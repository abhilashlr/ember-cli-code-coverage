...
function fixPath(relativePath, name, root, templateExtensions, isAddon, isModuleUnificationApp) {
  // Handle addons
  if (isAddon) {
    ...
    let root = isModuleUnificationApp ? 'src' : 'app';
    relativePath = relativePath.replace(name, root);
...
function CoverageInstrumenter(inputNode, options) {
  ...
  this._isModuleUnificationApp = options.isModuleUnificationApp;
...
CoverageInstrumenter.prototype.processString = function(content, relativePath) {
  ...
  relativePath = fixPath(relativePath, this._appName, this._appRoot, this._templateExtensions, this._isAddon, this._isModuleUnificationApp);
